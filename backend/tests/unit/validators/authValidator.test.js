...
describe("auth validator", () => {
    ...
    describe("registerValidator success", () => {
        ...
        it("trims the name and normalizes the email", async () => {
            const { errors, req } = await runValidation(registerValidator, {
                body: {
                    name: "  Jane Doe  ",
                    email: "  JANE@EXAMPLE.COM  ",
                    password: "Password123"
    ...
    describe("registerValidator validation", () => {
        it.each([
            ...
            "rejects a %s name", async (_, name) => {
                ...
                const { errors } = await runValidation(registerValidator, { body });
        ...
        it.each([[
            ...
            "rejects a password %s", async (_, password, expectedMessage) => {
                const { errors } = await runValidation(registerValidator, {
                    body: {
                        name: "Jane Doe",
                        email: "jane@example.com",
    ...
    describe("loginValidator success", () => {
        ...
        it("trims and normalizes the login email", async () => {
            const { errors, req } = await runValidation(loginValidator, {
                body: {
                    email: "  JANE@EXAMPLE.COM  ",
                    password: "Password123"
                }
    ...
    describe("loginValidator validation", () => {
        it.each([
            ...
            "rejects a %s email", async (_, email) => {
                ...
                const { errors } = await runValidation(loginValidator, { body });

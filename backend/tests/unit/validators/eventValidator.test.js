...
describe("event validator", () => {
    ...
    describe("createEventValidator success", () => {
        it("accepts a valid in-person event payload", async () => {
            const { errors } = await runValidation(createEventValidator, {
        ...
        it("trims event text fields", async () => {
            const { errors, req } = await runValidation(createEventValidator, {
                body: createValidEventBody({
                    title: "  Tech Meetup  ",
        ...
        it("accepts omitted optional event fields", async () => {
            ...
            const { errors } = await runValidation(createEventValidator, { body });
    ...
    describe("createEventValidator required fields", () => {
        it.each([
            ...
            "rejects a missing %s", async (field, expectedMessage) => {
                const body = createValidEventBody();
        ...
        it.each([
            ...
            "rejects an empty %s", async (field, expectedMessage) => {
                const body = createValidEventBody({
                    [field]: "   "
                });
    ...
    describe("Creation mode and location", () => {
        ...
        it.each([
            ...
            "requires location for an in-person event when %s", async (_, location) => {
                const body = createValidEventBody();
    ...
    describe("Creation date validation", () => {
        it.each([[
            ...
            "rejects an invalid %s", async (_, overrides, expectedMessage) => {
                const { errors } = await runValidation(createEventValidator, {
    ...
    describe("Creation participant limit", () => {
        ...
        it.each([
            ...
            "rejects a %s maximum participant count", async (_, maxParticipants) => {
                const { errors } = await runValidation(createEventValidator, {
                    body: createValidEventBody({
                        maxParticipants
                    })
    ...
    describe("updateEventValidator success", () => {
        it("accepts an empty update payload", async () => {
            const { errors } = await runValidation(updateEventValidator);

            expect(errors).toHaveLength(0);
        });

        it("accepts valid partial event updates", async () => {
            const { errors, req } = await runValidation(updateEventValidator, {
                body: {
                    title: "  Updated Event  ",
                    theme: "  Community  ",
                    mode: "online"
    ...
    describe("Nullable update fields", () => {
        ...
        it.each([
            ...
            "rejects a %s maximum participant update", async (_, maxParticipants) => {
                const { errors } = await runValidation(updateEventValidator, {
                    body: {
                        maxParticipants
                    }
    ...
    describe("getAllEventsValidator", () => {
        it("accepts an empty event query", async () => {
            const { errors } = await runValidation(getAllEventsValidator);

            expect(errors).toHaveLength(0);
        });

        it("accepts and sanitizes valid event filters", async () => {
            const { errors, req } = await runValidation(getAllEventsValidator, {
                query: {
                    creatorId: "12",
                    search: "  community  ",
                    type: "  Meetup  ",
        ...
        it.each([[
            ...
            "rejects an invalid %s filter", async (_, query, expectedMessage) => {
                const { errors } = await runValidation(getAllEventsValidator, { query });

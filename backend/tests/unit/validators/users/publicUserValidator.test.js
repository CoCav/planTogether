...
describe("public user validator", () => {
    ...
    describe("publicUserIdParamValidator", () => {
        ...
        it.each([
            ...
            "rejects a %s public user ID", async (_, id) => {
                ...
                const { errors } = await runValidation(publicUserIdParamValidator, { params });
    ...
    describe("getPublicUserEventsValidator", () => {
        it("accepts an empty query", async () => {
            const { errors } = await runValidation(getPublicUserEventsValidator);
        ...
        it("accepts valid shared filters, pagination and sorting", async () => {
            const { errors, req } = await runValidation(getPublicUserEventsValidator, {
                query: {
                    view: "created",
                    status: "upcoming",
                    creator: "  Jane Doe  ",
        ...
        it.each([[
            ...
            "rejects an invalid %s query", async (_, query, expectedMessage) => {
                const { errors } = await runValidation(getPublicUserEventsValidator, { query });
